...
| Crowdy Agent | `client.crowdyStudioAgent` + `context.playerHost`; Constructor `use_studio_agent`; platform + app policy | `platform/studio/StudioService.ts`, `ConstructPlayerHostAdapter.ts`. Setup writes policy. Tokens are platform-funded. |
