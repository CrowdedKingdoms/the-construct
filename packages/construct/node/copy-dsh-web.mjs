 ...
 * into the game's `public/dsh/` (the current working directory is the game)
 * before `vite dev` / `vite build`. Run it as `construct-copy-dsh`. `public/dsh/` is
...
const root = path.resolve(process.env.CONSTRUCT_APP_ROOT ?? process.cwd());
...
writeFileSync(
  ...
  `Copied by construct-copy-dsh from ${origin} on ${new Date().toISOString()}.\n` +
