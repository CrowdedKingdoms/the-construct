...
`,P={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"};function x(e){return e.replace(/[&<>"']/g,t=>P[t]??t)}function z(e,t){return`<label data-preview-source-option>
  ...
  </label>`}function B(e,t){return e.map(r=>({id:r.id,label:r.label,description:r.description,overlays:r.overlays.map(o=>new URL(o,t))}))}async function W(e){const t=new URL(location.href).searchParams.get(j);if(t===b)return[];const r=await fetch(e);if(!r.ok)throw new Error(`preview source chooser: fixture manifest returned ${String(r.status)}`);const o=E(await r.json()),s=[{id:b,label:"Empty environment",description:"Load only the base runtime to verify first launch and workspace creation.",overlays:[]},...B(o.fixtures,e),{id:M,label:"WebFS directory",description:"Requires directory access and will be available after the WebFS provider lands.",overlays:[],disabled:!0}];if(t!==null){const l=s.find(h=>h.id===t&&h.disabled!==!0);if(l===void 0)throw new Error(`preview source chooser: unknown or interactive source "${t}"`);return l.overlays}const n=document.getElementById("root");if(n===null)throw new Error("preview source chooser: missing #root");const i=o.defaultFixture??b,a=document.createElement("style");a.dataset.previewSourceStyle="",a.textContent=I,document.head.append(a);const d=document.createElement("main");d.dataset.previewSourceChooser="",d.innerHTML=`<form data-preview-source-card aria-labelledby="preview-source-title">
    ...
    </form>`,n.prepend(d);const c=d.querySelector("[data-preview-source-card]");if(c===null)throw new Error("preview source chooser: form was not rendered");const u=await new Promise((l,h)=>{c.addEventListener("submit",k=>{k.preventDefault();const v=new FormData(c).get("preview-source");typeof v=="string"?l(v):h(new Error("preview source chooser: no source selected"))},{once:!0})}),p=s.find(l=>l.id===u&&l.disabled!==!0);if(p===void 0)throw new Error(`preview source chooser: unavailable source "${u}"`);return d.remove(),a.remove(),p.overlays}function g(){return globalThis.__DSH_BOOT_READY__??=Promise.withResolvers()}function C(){g().promise.catch(()=>{})}async function D(e={}){C();const t=new URL(e.image??"vfs-image.tar.gz",document.baseURI),r=new URL(e.fixtureManifest??"fixtures.json",t);try{return{overlays:await W(r)}}catch(o){throw g().reject(o),o}}async function N(e,t){const r=g();r.promise.catch(()=>{});try{const o=new U(e);o.init(new URL(t?.image??"vfs-image.tar.gz",document.baseURI).href,(t?.overlays??[]).map(n=>new URL(n,document.baseURI).href));const s=await o.bootPayload();return globalThis.__DSH_TRANSPORT__={fetch:(n,i)=>o.fetch(n,i),openStream:(n,i,a)=>o.open(n,i,a),loadBundle:n=>o.loadBundle(n),ownsHost:!0},globalThis.__DSH_FILE_UPLOAD__={fetch:(n,i)=>o.fetch(n,i)},await O(s.injections,n=>o.loadBundle(n)),r.resolve(),{worker:e,tunnel:o,loadBundle:n=>o.loadBundle(n)}}catch(o){throw r.reject(o),o}}export{q as I,H as W,D as a,N as c};
//# sourceMappingURL=client-Dbm2RGSD.js.map
